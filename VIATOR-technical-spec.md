...
**Provider-bundle (since v0.1.6):** `sources.providers` is a list of objects, one per railway / transit operator. Each provider can contribute up to four URLs:

```json
{
  "id": "SNCF",                    // OTP feedId namespace prefix on stop_ids
  "label": "SNCF Trains",          // operator-facing display name
  "country_iso": "FR",             // ISO-2; gates session save against master_stations
  "timetable": {
    "format": "gtfs",              // gtfs | netex_nordic | netex_epip
    "url": "https://eu.ftp.opendatasoft.com/sncf/plandata/Export_OpenData_SNCF_GTFS_NewTripId.zip"
  },
  "gtfs_rt": {
    "alerts_url":             "https://proxy.transport.data.gouv.fr/.../sncf-gtfs-rt-alerts",
    "trip_updates_url":       "https://proxy.transport.data.gouv.fr/.../sncf-gtfs-rt-trip-updates",
    "vehicle_positions_url":  "https://..."
  },
  "mct_url":          "https://...",   // optional minimum-connection-times CSV
  "stations_csv_url": "https://..."    // optional provider stations list
}
```

Country-gate: at session-save time, the API checks that `master_stations` has at least one row with `country_iso = X` for every X declared by a provider. If not, save returns HTTP 409 with `{error: "missing_master_stations_for_countries", missing_countries: [...]}` — operator-driven; UI surfaces a "Import from Trainline" prompt. Operator clicks `/admin/master/stations → Refresh from Trainline`, then retries the save.

NeTEx-FR is intentionally excluded from the timetable formats — OTP can't read it. NeTEx-FR archives use the manual upload path (`declared_standard=NeTEx-FR-Horaires`), which dispatches to `inbox/<sid>/archive/` for compliance and never touches OTP.

**Multi-feed (since v0.1.4):** `sources.gtfs` is a list of `{id, url}` objects; one OTP graph contains all listed feeds. `id` becomes the OTP `feedId` namespace prefix on every stop_id from that feed (e.g. `SNCF:OCETrain-87271007`). Cross-operator transfers between physically-nearby stops are auto-generated by OTP at build time. Backward-compat: a string value is read as a single feed with `id="GTFS"`. Validation: `id` matches `/^[A-Z][A-Z0-9_-]{1,15}$/`, unique per session. **In v0.1.6 the v0.1.4 shape is auto-migrated to a single-provider bundle on first save via the new UI.**
