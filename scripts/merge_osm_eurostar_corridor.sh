#!/usr/bin/env bash
# Merge Geofabrik regional PBFs into one corridor-wide OSM file for a
# multi-country VIATOR session: ES + FR + BE + LU + NL + DE + AT + IT +
# LI + CH + UK (Eurostar SE-England subset).
#
...
#
# Output: ${OUTPUT_PATH:-/tmp/osm-merge/eurostar-corridor.osm.pbf}
#
# Plug the resulting file into a session via either:
#   - serve via nginx and set sources.osm_pbf=<URL>, OR
#   - drop into /data/inbox/<sid>/osm/osm.pbf and skip the URL refresh.
#
# Disk:   ~25 GB peak working space (raw downloads + merged output).
# Time:   ~30-60 min depending on bandwidth + CPU.
...
OUTPUT_PATH="${OUTPUT_PATH:-${WORK_DIR}/eurostar-corridor.osm.pbf}"

# Geofabrik regional extracts. Each entry is "<basename>|<url>".
# - Belgium / Netherlands / Luxembourg fetched separately. The
#   `benelux-latest.osm.pbf` composite was discontinued by Geofabrik
#   sometime before 2026-06 — the URL now serves an HTML "no such
#   extract" page that curl --fail (200 OK) happily saves as a corrupt
#   ~10 KB "PBF". We hit that the hard way; per-country fetches are the
#   only reliable path now.
# - switzerland = CH + LI (Liechtenstein is in the Switzerland extract).
# - great-britain = GB raw; we'll bbox-clip it below.
REGIONS=(
...
# Minimum byte size for a cached PBF to be trusted (any region). Below
# this, the cached file is almost certainly an HTML error page that
# curl saved without --fail catching it (a 200-OK error landing, e.g.
# Geofabrik serving their homepage when an extract no longer exists).
# Luxembourg is the smallest legit region at ~40 MB, so 5 MB is a
# comfortable lower bound that catches HTML stubs (~10 KB) without
# false-positiving on any genuine extract.
...
log "Phase 1/4 — downloading regional PBFs to ${WORK_DIR}/raw"
...
# Most regions get a verbatim copy into staged/. GB is the exception:
# bbox-clip the raw GB PBF down to the HS1 corridor. Everything else can
# stay raw — the session build's geo-crop polygon will later trim each
# country to its national outline anyway. (Doing the GB clip here keeps
# the merge cheap and the merged file's GB footprint tiny from the
# start, rather than carrying 1.5 GB of UK we don't want through the
# merge then trimming it later.)
for entry in "${REGIONS[@]}"; do
    name="${entry%%|*}"
...
log "    sources.osm_pbf=<URL> on the new session, OR"
log "  • Copy directly into the session's inbox:"
log "      cp ${OUTPUT_PATH} /data/inbox/<sid>/osm/osm.pbf"
