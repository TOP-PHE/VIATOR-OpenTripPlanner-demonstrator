...
**7. Per-session OTP containers all running** *(audit-2026-05 incident
2026-05-07)*

The web/worker/nginx/postgres healthchecks above don't cover the
**per-session OTP containers** generated by the orchestrator — and a
deploy that recreates web/worker can leave per-session containers in
`Exited (143)` state without restarting them. Verify all serving
sessions actually have a running OTP process.

```bash
# Sessions the DB thinks are serving:
docker exec viator-web-1 \
  python -c "from app.worker import _list_serving_sessions; print(_list_serving_sessions())"
# → ['fr-rail', 'eu-nap-network', '2026-W19_2026-W31', ...]

# Containers actually running:
docker ps --filter "name=^viator-otp-" --format "{{.Names}}\t{{.Status}}"
# Expected: one Up container per serving session, no Exited.
```

The two lists must match. If a session is "serving" in the DB but its
container is missing or exited:

- **Container exists but Exited** (e.g. `Exited (143)` from a previous
  SIGTERM during a deploy): `cd /opt/viator/docker && sudo docker
  compose up -d` — restarts any defined-but-stopped service.
  The worker's orphan-cleanup pass (post-audit-2026-05 #25) also picks
  these up on the next reload tick, but `compose up -d` is faster.

- **Container doesn't exist at all**: re-promote the session from the
  admin UI. The orchestrator regenerates the compose entry and the
  worker spawns the container; OTP loads the existing graph from disk
  in ~90s without rebuilding. The graph file lives in the `viator_graphs`
  named volume and survives container deletion.

If you see *Exited* containers for sessions that have been **deleted**
in the UI, those are the orphan-cleanup gap from audit-2026-05 #25.
Pre-fix workaround: `docker rm <container>` manually. Post-fix
(merged): the worker's reload tick reaps them automatically.
